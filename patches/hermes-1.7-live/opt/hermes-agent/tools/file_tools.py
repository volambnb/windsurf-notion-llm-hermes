...
_ACTIVE_CODE_PLAN_FILE = Path(os.environ.get("HERMES_ACTIVE_CODE_PLAN_FILE", "/root/.hermes/active_code_plan.json"))
_PLANS_ROOTS = tuple(Path(p).expanduser().resolve() for p in ("/root/.hermes/plans", "~/.hermes/plans"))


def _active_code_plan_path() -> Path | None:
    if not _ACTIVE_CODE_PLAN_FILE.exists():
        return None
    try:
        data = json.loads(_ACTIVE_CODE_PLAN_FILE.read_text(encoding="utf-8"))
        current = data.get("current") if isinstance(data, dict) else None
        active = current.get("active_plan_file") if isinstance(current, dict) else None
        return Path(str(active)).expanduser().resolve() if active else None
    except Exception:
        return None


def _read_active_plan_direct(resolved_path: Path, offset: int, limit: int) -> str | None:
    active_path = _active_code_plan_path()
    try:
        rp = resolved_path.resolve()
    except Exception:
        rp = resolved_path
    if active_path is None or rp != active_path or not rp.exists() or not rp.is_file():
        return None
    try:
        ...
        return json.dumps({"error": f"Failed to read active plan directly: {exc}"}, ensure_ascii=False)


def _stale_plan_read_error(resolved_path: Path) -> str | None:
    """Block reads of old Hermes plan files when /code has an active plan."""
    try:
        rp = resolved_path.resolve()
    except Exception:
        rp = resolved_path
    if not any(str(rp).startswith(str(root) + os.sep) or rp == root for root in _PLANS_ROOTS):
        return None
    if not _ACTIVE_CODE_PLAN_FILE.exists():
        return None
    active_path = _active_code_plan_path()
    if active_path is None or rp == active_path:
        return None
    return (
        "stale plan denied; use Current Plan File only. "
        f"Requested plan: {rp}. Current Plan File: {active_path}. "
        "Do not list/search/read other files under ~/.hermes/plans."
    )
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        stale_plan_error = _stale_plan_read_error(_resolved)
        if stale_plan_error:
            return json.dumps({"error": stale_plan_error}, ensure_ascii=False)

        active_plan_direct = _read_active_plan_direct(_resolved, offset, limit)
        if active_plan_direct is not None:
            return active_plan_direct
